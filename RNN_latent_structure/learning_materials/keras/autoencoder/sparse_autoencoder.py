...
This autoencoder builds on top of the original by promoting sparsity within the
encoded layer (responsible for the low-dimensional representation). The idea then
is that this helps generates possibly more orthogonal? internal representations.
We do so by adding an activity regulator to this encoding layer
'''
...
use_sequential = True # use Sequential() model class vs Model() class; both generate identical results
...
# Here we use an L1 activity regularizer with lambda = 10e-15
encoded = Dense(encoding_dim, activation='relu', activity_regularizer = regularizers.l1(10e-15))(input_img)
...
model = Sequential()
model.add(Dense(encoding_dim, input_dim = 784, activation='relu', activity_regularizer = regularizers.l1(10e-15)))
