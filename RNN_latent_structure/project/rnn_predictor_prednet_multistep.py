...
This generates a dt = 1 timestep predictor model that will act as the initialization for a multistep prediction model
'''
...
assert args.load is not None and args.dt is not None, "You must load a single step model to finetune (or a previous multistep one) AND provide a timestep for prediction (use --load or --dt)"
...
if args.load is not None:
    ...
    if args.name is None: model_name = 'finetune'
    model_filename = Path(model_name + "_dt_"  + str(args.dt) + ".h5")
    model_filename = 'models' / model_filename 
...
dt = 1 # TODO: is this always the case?
...
singlestep_model = load_model(args.load, {"PredNet": prednet.PredNet})
...
model.set_weights(singlestep_model.weights)
